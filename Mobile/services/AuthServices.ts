...
import { RegisterUser, AuthResponse, LoginUser, User } from "~/lib/Types";

class AuthService {
  // Método para registrar un nuevo usuario
  async register(user: RegisterUser): Promise<AuthResponse> {
    try {
      const response = await apiService.post<AuthResponse, RegisterUser>(
        "/user/register",
        user
      );
      return response;
      ...
      throw new Error(
        error.response.data.message || "Error registrando el usuario"
      );
    }
  }
  ...
  async login(user: LoginUser): Promise<AuthResponse> {
    try {
      const response = await apiService.post<AuthResponse, LoginUser>(
        "/user/login",
        user
      );
  ...
  async GetUserInfo(): Promise<User> {
    try {
      const response = await apiService.get<User>("/user/list");
      // Establece el token en el servicio API si la autenticación es exitosa
      return response;
    } catch (error) {
      throw new Error(error.response.data.message || "Error iniciando sesión");
